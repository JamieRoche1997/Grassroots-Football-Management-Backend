...
def manage_lineups():
    try:
        if request.method in ['POST', 'PATCH', 'DELETE']:
            ...
            data = request.args
        ref = fixture_ref(data['clubName'], data['ageGroup'], data['division'], data['matchId'])

        if request.method == 'POST':
            ref.collection('lineups').document('home').set({"lineup": data.get('homeTeamLineup', {})})
            ref.collection('lineups').document('away').set({"lineup": data.get('awayTeamLineup', {})})
            ...
            if home_update:
                ref.collection('lineups').document('home').update({"lineup": home_update})
            if away_update:
                ref.collection('lineups').document('away').update({"lineup": away_update})
            ...
            home_lineup = ref.collection('lineups').document('home').get().to_dict() or {}
            away_lineup = ref.collection('lineups').document('away').get().to_dict() or {}
            return jsonify({"homeTeamLineup": home_lineup.get('lineup', {}), "awayTeamLineup": away_lineup.get('lineup', {})}), 200
...
def manage_player_ratings():
    try:
        ...
        if request.method == 'POST':
            ...
            rating_ref = (db.collection("clubs").document(club_name)
                      .collection("ageGroups").document(age_group)
                      .collection("divisions").document(division)
                      .collection("fixtures").document(match_id)
                      .collection("playerRatings").document(player_email))
            ...
            ratings = []
            ratings_ref = (db.collection("clubs").document(clubName)
                        .collection("ageGroups").document(ageGroup)
                        .collection("divisions").document(division)
